...
static void rx_task(void *arg)
{
	...
	while (1)
	{
		...
		if (rxBytes > 0)
		{
			...
			char** sentences = split_by_2_separators((char*)data, "$GPGGA", "*", 0, 0);


			//split_by_2_separators(sentences, (char*)data, "$GNGGA", "*", 0);


			char** raw_coords = split_to_sentences(sentences[0], ",", 0, 0);
		    for (int i = 0; i < 1; i++)
		    {
			...
			snprintf(display_text_4, sizeof(display_text_4), "Height: %.2f", height);
